...
<div  >
        ...
        <label style="font-family: Amiri; font-size: 24pt; margin-right: 12px;" >{{$arr['cus']->CompanyName}}</label>
    <br>
        <label style="font-family: Amiri; font-size: 18pt;margin-right: 12px;">{{$arr['cus']->CompanyNameSuffix}}</label>
  ...
  <table  width="100%"   align="right" style="border: none;">
    ...
    @foreach($arr['orderdetail'] as  $item)
      <tr >
          ...
          <td style=" text-align: right;"> {{ $item->Item->item_name }} </td>
          <td style="text-align: center;"> {{ $item->quant }} </td>
          <td style=" text-align: right;"> {{ $item->price }} </td>
          <td style=" text-align: right;"> {{ $item->subtot }}</td>
      </tr>
